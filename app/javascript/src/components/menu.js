...
import {Util, EventHandler} from "./index";
// Class definition
var Menu = function (element, options) {
    ...
    var _init = function () {
        ...
        // Set initialized
        the.element.setAttribute('data-controller', 'components--menu');
    ...
    var _link = function (element, e) {
        if (EventHandler.trigger(the.element, 'kt.menu.link.click', the) === false) {
            return;
        }
        ...
        EventHandler.trigger(the.element, 'kt.menu.link.clicked', the);
    }
    ...
    var _setTriggerElement = function () {
        ...
        } else if (the.element.parentNode && Util.child(the.element.parentNode, '[data-kt-menu-trigger]')) {
            the.triggerElement = Util.child(the.element.parentNode, '[data-kt-menu-trigger]');
        }
    ...
    var _hasItemSub = function (item) {
        return (Util.hasClass(item, 'menu-item') && item.hasAttribute('data-kt-menu-trigger'));
    }
    ...
    var _hideAccordions = function (item) {
        var itemsToHide = Util.findAll(the.element, '.show[data-kt-menu-trigger]');
        var itemToHide;
...
Menu.dropdownHandler = function (e) {
    var items = document.querySelectorAll('.show.menu-dropdown[data-kt-menu-trigger]');
    var menu;
    var item;
    var sub;
...
// Global handlers
Menu.initGlobalHandlers = function () {
    // Dropdown handler
    document.addEventListener("click", Menu.dropdownHandler);

    // Sub toggle handler(update)
    Util.on(document.body, '.menu-item[data-kt-menu-trigger] > .menu-link, [data-kt-menu-trigger]:not(.menu-item):not([data-kt-menu-trigger="auto"])', 'click', function (e) {
    ...
    // Link handler
    Util.on(document.body, '.menu-item:not([data-kt-menu-trigger]) > .menu-link', 'click', function (e) {
        var menu = Menu.getInstance(this);
    ...
    // Dismiss handler
    Util.on(document.body, '[data-kt-menu-dismiss="true"]', 'click', function (e) {
        var menu = Menu.getInstance(this);
    ...
    // Mouseover handler
    Util.on(document.body, '[data-kt-menu-trigger], .menu-sub', 'mouseover', function (e) {
        var menu = Menu.getInstance(this);
    ...
    // Mouseout handler
    Util.on(document.body, '[data-kt-menu-trigger], .menu-sub', 'mouseout', function (e) {
        var menu = Menu.getInstance(this);
...
Menu.resize = function () {
    ...
    Util.throttle(timer, function () {
        ...
        var elements = document.querySelectorAll('[data-controller="components--menu"]');
...
// Global instances
Menu.createInstances = function (selector = '[data-controller="components--menu"]') {
    // Initialize menus
    var elements = document.querySelectorAll(selector);
    if (elements && elements.length > 0) {
        for (var i = 0, len = elements.length; i < len; i++) {
            let menuInstance = Menu.getInstance(elements[i])
            if (!menuInstance) {
                menuInstance = new Menu(elements[i]);
            }
        }
    }
}
...
Menu.bootstrap = () => {
    ...
    Menu.createInstances('[data-controller="components--menu"]')
}
